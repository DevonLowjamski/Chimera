...
namespace ProjectChimera.Systems.Progression
{
    ...
    public class OfflineProgressionArchitecture : MonoBehaviour
    {
        ...
        private void InitializeOfflineProgression()
        {
            ...
            Debug.Log("[OfflineProgressionArchitecture] Offline progression system initialized");
        }
        ...
        public void RegisterProvider(string providerId, IOfflineProgressionProvider provider, float weight = 1.0f)
        {
            ...
            Debug.Log($"[OfflineProgressionArchitecture] Registered offline progression provider: {providerId} (weight: {weight})");
        }
        ...
        public void UnregisterProvider(string providerId)
        {
            if (_progressionProviders.ContainsKey(providerId))
            {
                ...
                Debug.Log($"[OfflineProgressionArchitecture] Unregistered offline progression provider: {providerId}");
            }
        }
        ...
        private void LoadLastOnlineTime()
        {
            ...
            {
                Debug.Log($"[OfflineProgressionArchitecture] Last online time loaded: {_lastOnlineTime:yyyy-MM-dd HH:mm:ss}");
            }
            else
            {
        ...
        private void SaveLastOnlineTime()
        {
            ...
            {
                Debug.Log($"[OfflineProgressionArchitecture] Last online time saved: {_lastOnlineTime:yyyy-MM-dd HH:mm:ss}");
            }
        }
        ...
        public async Task<OfflineProgressionSession> ProcessOfflineProgressionAsync(TimeSpan offlineTime)
        {
            ...
            {
                Debug.LogWarning("[OfflineProgressionArchitecture] Offline progression already in progress");
                return null;
            }
            ...
            try
            {
                ...
                if (_enableProgressionLogging)
                {
                    Debug.Log($"[OfflineProgressionArchitecture] Processing offline progression for {cappedOfflineTime.TotalHours:F2} hours");
                }
            ...
            {
                Debug.LogError($"[OfflineProgressionArchitecture] Offline progression error: {ex.Message}");
        ...
        private async Task ProcessProvidersAsync(OfflineProgressionSession session)
        {
            ...
            {
                Debug.LogWarning($"[OfflineProgressionArchitecture] Provider processing timed out after {_processingTimeLimit}s");
            }
        ...
        private async Task<OfflineProgressionResult> ProcessProviderAsync(string providerId, IOfflineProgressionProvider provider, OfflineProgressionSession session)
        {
            ...
            catch (Exception ex)
            {
                ...
                Debug.LogError($"[OfflineProgressionArchitecture] Provider {providerId} calculation failed: {ex.Message}");
            }
            finally
            {
        ...
        private async Task ApplyProgressionResultAsync(OfflineProgressionResult result)
        {
            ...
            try
            {
                ...
                if (_enableProgressionLogging)
                {
                    Debug.Log($"[OfflineProgressionArchitecture] Applied progression for {result.ProviderId}");
                }
            }
            catch (Exception ex)
            {
                Debug.LogError($"[OfflineProgressionArchitecture] Failed to apply progression for {result.ProviderId}: {ex.Message}");
        ...
        private void ValidateProgressionResults(OfflineProgressionSession session)
        {
            ...
            {
                Debug.LogWarning($"[OfflineProgressionArchitecture] Validation issues found: {string.Join(", ", validationIssues)}");
            }
        }
        ...
        public async Task<OfflineProgressionSession> SimulateOfflineProgressionAsync(TimeSpan simulatedOfflineTime)
        {
            ...
            Debug.Log($"[OfflineProgressionArchitecture] Simulating offline progression for {simulatedOfflineTime.TotalHours:F2} hours");
        ...
        public void ClearProgressionHistory()
        {
            ...
            _sessionMetrics = new OfflineProgressionMetrics();
            Debug.Log("[OfflineProgressionArchitecture] Progression history cleared");
        }
    ...
    /// <summary>
    /// Interface for offline progression providers
    /// </summary>

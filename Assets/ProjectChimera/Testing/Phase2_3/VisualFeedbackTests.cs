...
namespace ProjectChimera.Testing.Phase2_3
{
    ...
    public class VisualFeedbackTests : MonoBehaviour
    {
        ...
        private void LogTestCategory(string categoryName)
        {
            ...
            {
                ChimeraLogger.Log($"[VisualFeedbackTests] === {categoryName} ===");
            }
        }
        ...
        private void LogResults()
        {
            ...
            ChimeraLogger.Log($"[VisualFeedbackTests] Tests completed: {_passedTests}/{_totalTests} passed");

            if (allTestsPassed)
            {
                ChimeraLogger.Log("✅ Visual Feedback Tests - ALL TESTS PASSED!");
            }
            else
            {
                ChimeraLogger.LogWarning($"⚠️ Visual Feedback Tests - {_totalTests - _passedTests} tests failed");
            }
        }
    }
}

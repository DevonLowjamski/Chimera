...
namespace ProjectChimera.UI.Managers
{
    ...
    /// </summary>
    public class ConstructionPaletteManager : MonoBehaviour, ITickable
    {
        ...
        {
            ProjectChimera.Core.Logging.ChimeraLogger.Log("[ConstructionPaletteManager] Registered with UpdateOrchestrator");
        }

        public void OnUnregistered()
        {
            ProjectChimera.Core.Logging.ChimeraLogger.Log("[ConstructionPaletteManager] Unregistered from UpdateOrchestrator");
        }
        ...
        private void ResolveDependencies()
        {
            if (_palettePanel == null)
            {
                ...
                if (_palettePanel == null)
                {
                    ProjectChimera.Core.Logging.ChimeraLogger.LogError("[ConstructionPaletteManager] ConstructionPalettePanel not registered in DI container. Explicit dependency injection required for testing.");
                }
            }
            ...
            if (_placementController == null)
            {
                ...
                if (_placementController == null)
                {
                    ProjectChimera.Core.Logging.ChimeraLogger.LogError("[ConstructionPaletteManager] GridPlacementController not registered in DI container. Construction placement will not function properly.");
                }
            }
            ...
            if (_uiManager == null)
            {
                ...
                if (_uiManager == null)
                {
                    ProjectChimera.Core.Logging.ChimeraLogger.LogWarning("[ConstructionPaletteManager] UIManager not registered in DI container. Some UI integration features may not work.");
                }
            }
        }
        ...
        private void InitializePaletteManager()
        {
            ...
            {
                _ = DiscoverSchematicsAsync(); // Fire and forget - don't block initialization
            }
        ...
        /// </summary>
        private async Task DiscoverSchematicsAsync()
        {
            ...
            if (addressablesService == null)
            {
                ...
                if (addressablesService == null)
                {
                    ChimeraLogger.LogWarning("[ConstructionPaletteManager] AddressablesInfrastructure not found. Creating instance.");
            ...
            try
            {
                // Note: Resources.LoadAll pattern needs individual asset loading
                // For now, we'll use predefined schematic addresses
                var schematicAddresses = new[] {
                    "Schematics/Construction/Walls",
                    "Schematics/Construction/Doors",
                    "Schematics/Construction/Windows",
                    "Schematics/Equipment/Lights",
                    "Schematics/Equipment/HVAC",
                    "Schematics/Equipment/Irrigation",
                    "Schematics/Equipment/Tables"
                };
                ...
                foreach (var address in schematicAddresses)
                {
                    ...
                    {
                        ProjectChimera.Core.Logging.ChimeraLogger.LogWarning($"[ConstructionPaletteManager] Could not load schematic '{address}': {ex.Message}");
                    }
                }

                ChimeraLogger.Log($"[ConstructionPaletteManager] Discovered {_discoveredSchematics.Count} schematics via Addressables");
            }
            catch (System.Exception ex)
            {
                ProjectChimera.Core.Logging.ChimeraLogger.LogError($"[ConstructionPaletteManager] Failed to discover schematics: {ex.Message}");
            }
        }
        ...
        {
            ProjectChimera.Core.Logging.ChimeraLogger.Log($"[ConstructionPaletteManager] Schematic selected: {schematic.SchematicName}");
        ...
        {
            ProjectChimera.Core.Logging.ChimeraLogger.Log($"[ConstructionPaletteManager] Tab changed to: {tab}");
        ...
        {
            ProjectChimera.Core.Logging.ChimeraLogger.Log("SYSTEM", $"[ConstructionPaletteManager] Item selected: {item.Name} ({item.Type})");
        }
        ...
        {
            ProjectChimera.Core.Logging.ChimeraLogger.Log($"[ConstructionPaletteManager] Schematic applied: {schematic.SchematicName} at {position} with {objects.Count} objects");
        ...
        public void SetAutoDiscovery(bool enabled)
        {
            ...
            {
                // DiscoverSchematics(); // Temporarily commented out - method not implemented
            }
        }

...
namespace ProjectChimera.Systems.Services.SpeedTree.Environmental
{
    ...
    public class WindSystem : MonoBehaviour
    {
        ...
        {
            ChimeraLogger.Log("[WindSystem] Initializing SpeedTree wind system...");
            ...
            ChimeraLogger.Log("[WindSystem] SpeedTree wind system initialized");
        }

        public void Shutdown()
        {
            ChimeraLogger.Log("[WindSystem] Shutting down SpeedTree wind system...");

            _windZones.Clear();
            _activeWindZones.Clear();

            ChimeraLogger.Log("[WindSystem] SpeedTree wind system shutdown complete");
        }
        ...
        public void RegisterWindZone(WindZone windZone)
        {
            ...
            try
            {
                ...
                ChimeraLogger.Log($"[WindSystem] Registered wind zone: {windZone.name}");
            }
            catch (Exception ex)
            {
                ChimeraLogger.LogError($"[WindSystem] Failed to register wind zone: {ex.Message}");
            }
        }
        ...
        public void UnregisterWindZone(WindZone windZone)
        {
            ...
            ChimeraLogger.Log($"[WindSystem] Unregistered wind zone: {windZone.name}");
        }
        ...
        public void UpdateWindZone(WindZone windZone)
        {
            ...
            {
                ChimeraLogger.LogError($"[WindSystem] Failed to update wind zone {windZone.name}: {ex.Message}");
            }
        }
        ...
        public void UpdateGlobalWind()
        {
            ...
            {
                ChimeraLogger.LogError($"[WindSystem] Failed to update global wind: {ex.Message}");
            }
        }
        ...
        private void UpdateWindGusts()
        {
            ...
            if (Random.value < 0.1f) // 10% chance per gust interval
            {
                ...
                ChimeraLogger.Log($"[WindSystem] Wind gust: {gustStrength}");
            }
        }
        ...
        private void ApplyWindToRenderer(Renderer renderer, float strength, Vector3 direction)
        {
            ...
            {
                ChimeraLogger.LogError($"[WindSystem] Failed to apply wind to renderer: {ex.Message}");
            }
        }
        ...
        public void Tick(float deltaTime)
        {
            ...
            {
                ChimeraLogger.LogError($"[WindSystem] Error in wind update loop: {ex.Message}");
            }
        }

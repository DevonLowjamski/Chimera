    ...
    public class SaveStorage : MonoBehaviour
    {
        ...
        private void InitializeSystem()
        {
            try
            {
                ...
                _isInitialized = true;
                ChimeraLogger.Log("[SaveManager] Save system initialized successfully");
            }
            catch (Exception ex)
            {
                ChimeraLogger.LogError($"[SaveManager] Failed to initialize save system: {ex.Message}");
            }
        }
        ...
        private async void InitializeCloudSync()
        {
            try
            {
                ...
                if (_cloudStorageProvider != null)
                {
                    ...
                    ChimeraLogger.Log($"[SaveManager] Cloud sync initialized: {_cloudProvider} - {_cloudSyncStatus}");
                }
            }
            catch (Exception ex)
            {
                ...
                OnCloudSyncStatusChanged?.Invoke(_cloudSyncStatus);
                ChimeraLogger.LogError($"[SaveManager] Cloud sync initialization failed: {ex.Message}");
            }
        }
        ...
        {
            ChimeraLogger.Log($"[SaveStorage] {message}");
        }

        private void LogWarning(string message)
        {
            ChimeraLogger.LogWarning($"[SaveStorage] {message}");
        }

        private void LogError(string message)
        {
            ChimeraLogger.LogError($"[SaveStorage] {message}");
        }

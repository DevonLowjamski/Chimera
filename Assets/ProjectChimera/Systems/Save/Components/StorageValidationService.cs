...
namespace ProjectChimera.Systems.Save.Components
{
    ...
    public class StorageValidationService : MonoBehaviour
    {
        ...
        private ProjectChimera.Data.Save.HashAlgorithmType _hashAlgorithm = ProjectChimera.Data.Save.HashAlgorithmType.SHA256;
        ...
        public void Initialize(bool enableIntegrityChecking, bool enableCorruptionDetection,
            bool enableContentValidation, ProjectChimera.Data.Save.HashAlgorithmType hashAlgorithm, int maxRetries, float timeout)
        {
        ...
        public async Task<string> GenerateIntegrityHashAsync(byte[] data)
        {
            try
            {
                System.Security.Cryptography.HashAlgorithm hashAlgorithm = _hashAlgorithm switch
                {
                    ...
                    ProjectChimera.Data.Save.HashAlgorithmType.SHA1 => SHA1.Create(),
        ...
        private void LogInfo(string message)
        {
            if (_enableDebugLogging)
                ChimeraLogger.Log($"[StorageValidationService] {message}");
        }

        private void LogWarning(string message)
        {
            if (_enableDebugLogging)
                ChimeraLogger.LogWarning($"[StorageValidationService] {message}");
        }

        private void LogError(string message)
        {
            if (_enableDebugLogging)
                ChimeraLogger.LogError($"[StorageValidationService] {message}");
        }
    }

...
namespace ProjectChimera.Systems.UI
{
    ...
    public class CultivationDashboard : MonoBehaviour, IUIUpdatable
    {
        ...
        private void Start()
        {
            // CultivationManager is a ChimeraManager, get from ServiceContainer
        ...
        private void RefreshDashboardData()
        {
            ...
            _currentData = new CultivationDashboardData
            {
                ...
                SystemStatus = MapCultivationSystemStatus(_cultivationManager.GetSystemStatus()),
        ...
        /// <summary>
        /// Map cultivation system status from cultivation namespace to UI namespace
        /// </summary>
        private ProjectChimera.Systems.UI.CultivationSystemStatus MapCultivationSystemStatus(
            ProjectChimera.Systems.Cultivation.CultivationSystemStatus status)
        {
            if (status == null || !status.IsSystemActive)
                return ProjectChimera.Systems.UI.CultivationSystemStatus.Critical;

            // Determine status based on health metrics
            float healthyRatio = status.TotalPlants > 0 ? (float)status.HealthyPlants / status.TotalPlants : 0f;

            if (healthyRatio >= 0.8f && status.AverageHealth >= 80f)
                return ProjectChimera.Systems.UI.CultivationSystemStatus.Healthy;
            else if (healthyRatio >= 0.6f && status.AverageHealth >= 60f)
                return ProjectChimera.Systems.UI.CultivationSystemStatus.Warning;
            else
                return ProjectChimera.Systems.UI.CultivationSystemStatus.Critical;
        }

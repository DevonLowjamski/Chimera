...
namespace ProjectChimera.Systems.Cultivation
{
    ...
    public class PlantPhysiology : ChimeraMonoBehaviour, ITickable
    {
        [Header("Plant Data")]
        ...
        UpdateOrchestrator.Instance?.RegisterTickable(this);
            ...
            {
                // Use reflection to find PlantVisualInstance without assembly dependency
                var visualComponents = GetComponents<MonoBehaviour>();
                foreach (var component in visualComponents)
                {
                    if (component.GetType().Name == "PlantVisualInstance")
                    {
                        _visualInstance = component;
                        break;
                    }
                }
            }
            ...
            if (_plantData == null)
            {
                ChimeraLogger.LogWarning($"[PlantPhysiology] Plant '{name}' has no PlantInstanceSO assigned.", this);
            }

            if (_growthCalculation == null)
            {
                ChimeraLogger.LogWarning($"[PlantPhysiology] Plant '{name}' has no GrowthCalculationSO assigned.", this);
            }
        }
            ...
            public void Tick(float deltaTime)
    {
            ...
            float effectiveDeltaTime = GetEffectiveDeltaTime();
            _accumulatedGrowthTime += effectiveDeltaTime;
        ...
        public void InitializePlant(PlantInstanceSO plantData, GrowthCalculationSO growthCalculation = null)
        {
            ...
            if (_visualInstance != null && _plantData != null)
            {
                ...
                if (initializeMethod != null)
                {
                    ...
                    {
                        ChimeraLogger.LogWarning($"Failed to initialize visual instance: {ex.Message}");
                    }
                }
            }
        }
        ...
        public void Water(float amount = 0.5f)
        {
            ...
            ChimeraLogger.Log($"[PlantPhysiology] Watered plant '{_plantData.PlantName}' with {amount * 100f}% water.");
        }
        ...
        public void Feed(float amount = 0.4f)
        {
            ...
            ChimeraLogger.Log($"[PlantPhysiology] Fed plant '{_plantData.PlantName}' with {amount * 100f}% nutrients.");
        }
        ...
        public void Train(string trainingType)
        {
            ...
            ChimeraLogger.Log($"[PlantPhysiology] Applied '{trainingType}' training to plant '{_plantData.PlantName}'.");
        }
        ...
        public bool Harvest()
        {
            ...
            ChimeraLogger.Log($"[PlantPhysiology] Harvested plant '{_plantData.PlantName}': {yield:F1}g at {potency:F1}% potency");
        ...
        private void HandlePhysiologyChanges(float previousHeight, float previousHealth, PlantGrowthStage previousStage)
        {
            ...
            if (_plantData.OverallHealth != previousHealth)
            {
                ...
                if (previousHealth > 0f && _plantData.OverallHealth <= 0f)
                {
                    ...
                    ChimeraLogger.LogWarning($"[PlantPhysiology] Plant '{_plantData.PlantName}' has died!");
                }
            }
            ...
            if (_plantData.CurrentGrowthStage != previousStage)
            {
                ...
                ChimeraLogger.Log($"[PlantPhysiology] Plant '{_plantData.PlantName}' transitioned from {previousStage} to {_plantData.CurrentGrowthStage}");
            }
        }
        ...
        private void UpdateVisualization()
        {
            ...
            if (_visualInstance != null)
            {
                ...
                if (updateMethod != null)
                {
                    ...
                    {
                        ChimeraLogger.LogWarning($"Failed to update visual parameters: {ex.Message}");
                    }
                }
            }
        ...
        private void CheckCriticalConditions()
        {
            ...
            if (NeedsWater)
            {
                ChimeraLogger.LogWarning($"[PlantPhysiology] Plant '{_plantData.PlantName}' critically needs water! ({_plantData.WaterLevel * 100f:F0}%)");
            }

            if (NeedsNutrients)
            {
                ChimeraLogger.LogWarning($"[PlantPhysiology] Plant '{_plantData.PlantName}' critically needs nutrients! ({_plantData.NutrientLevel * 100f:F0}%)");
            }

            // Check for harvest readiness
            if (IsReadyForHarvest)
            {
                ChimeraLogger.Log($"[PlantPhysiology] Plant '{_plantData.PlantName}' is ready for harvest!");
            }
        }

...
namespace ProjectChimera.Systems.Cultivation
{
    ...
    /// </summary>
    public class PlantGrowthStageManager : MonoBehaviour, IChimeraManager
    {
        ...
        public void Shutdown()
        {
            ...
            if (_enableDebugLogs)
            {
                Debug.Log("[PlantGrowthStageManager] Shutdown complete.");
            }
        }
        ...
        private void Awake()
        {
            ...
            if (_enableDebugLogs)
            {
                Debug.Log("[PlantGrowthStageManager] Awakening...");
            }
        }
        ...
        private async void InitializeAsync()
        {
            try
            {
                ...
                if (_enableDebugLogs)
                {
                    Debug.Log("[PlantGrowthStageManager] Core systems initialized.");
                }
                ...
                {
                    Debug.LogWarning("[PlantGrowthStageManager] ITimeManager not found. Using Unity Time.");
                }
                ...
                if (_enableDebugLogs)
                {
                    Debug.Log("[PlantGrowthStageManager] Core initialization complete.");
                }
                ...
                if (_enableDebugLogs)
                {
                    Debug.Log("[PlantGrowthStageManager] Initialization complete.");
                }
            }
            catch (Exception ex)
            {
                Debug.LogError($"[PlantGrowthStageManager] Initialization failed: {ex.Message}");
                IsInitialized = false;
            }
        }
        ...
        public bool RegisterPlant(PlantInstanceSO plantInstance)
        {
            ...
            {
                Debug.LogWarning("[PlantGrowthStageManager] Cannot register null plant instance.");
                return false;
            }
            ...
            {
                Debug.LogWarning("[PlantGrowthStageManager] Cannot register plant with empty ID.");
                return false;
            }
            ...
            if (_plantStageTracking.ContainsKey(plantId))
            {
                if (_enableDebugLogs)
                {
                    Debug.Log($"[PlantGrowthStageManager] Plant {plantId} already registered. Updating data.");
                }
            }
            ...
            var stageData = new PlantStageData
            {
                ...
                StageDuration = GetStageDuration(plantInstance.CurrentGrowthStage, plantInstance.Genotype),
                StageProgress = 0f,
                IsStageComplete = false,
                EnvironmentalHistory = new List<EnvironmentSnapshot>(),
            ...
            if (_enableDebugLogs)
            {
                Debug.Log($"[PlantGrowthStageManager] Registered plant {plantId} at stage {stageData.CurrentStage}");
            }
        ...
        public bool UnregisterPlant(string plantId)
        {
            ...
            {
                Debug.Log($"[PlantGrowthStageManager] Unregistered plant {plantId}");
            }
        ...
        public bool ForceStageTransition(string plantId, PlantGrowthStage targetStage)
        {
            if (!_plantStageTracking.TryGetValue(plantId, out var stageData))
            {
                Debug.LogWarning($"[PlantGrowthStageManager] Cannot force transition for unregistered plant {plantId}");
                return false;
            }
            
            if (stageData.CurrentStage == targetStage)
            {
                Debug.LogWarning($"[PlantGrowthStageManager] Plant {plantId} is already at stage {targetStage}");
                return false;
            }
        ...
        private void UpdatePlantStage(PlantStageData stageData, float currentTime)
        {
            ...
            if (_enableStressDetection)
            {
                ...
                {
                    Debug.LogWarning($"[PlantGrowthStageManager] Plant {stageData.PlantId} is experiencing stress");
                }
            }
        }
        ...
        private bool ExecuteStageTransition(PlantStageData stageData, PlantGrowthStage newStage, bool forced)
        {
            ...
            if (!forced && !CanTransitionToStage(oldStage, newStage))
            {
                if (_enableDebugLogs)
                {
                    Debug.LogWarning($"[PlantGrowthStageManager] Invalid transition from {oldStage} to {newStage} for plant {stageData.PlantId}");
                }
                return false;
            }
            ...
            if (_enableDebugLogs)
            {
                Debug.Log($"[PlantGrowthStageManager] Plant {stageData.PlantId} transitioned from {oldStage} to {newStage} (Forced: {forced})");
            }
        ...
        private void PerformCleanup()
        {
            ...
            {
                Debug.Log($"[PlantGrowthStageManager] Cleaned up {plantsToRemove.Count} invalid plant entries");
            }
        }
        ...
        private void LogPerformanceMetrics()
        {
            ...
            Debug.Log($"[PlantGrowthStageManager] Performance - Tracked: {stats.TrackedPlants}, Queue: {stats.PlantsInQueue}, Processed this frame: {_plantsProcessedThisFrame}");
        }
    ...
    public class PlantStageData
    {
        ...
        public float StageProgress;
        public bool IsStageComplete;
        public List<EnvironmentSnapshot> EnvironmentalHistory;

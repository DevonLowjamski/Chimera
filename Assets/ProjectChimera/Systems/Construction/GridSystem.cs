...
namespace ProjectChimera.Systems.Construction
{
    ...
    public class GridSystem : MonoBehaviour, ITickable
    {
        ...
        private void InitializeModularSystem()
        {
            ...
            {
                ChimeraLogger.Log("[GridSystem] Modular grid system initialized successfully");
            });
        ...
        private void WireUpEvents()
        {
            ...
            _placement.OnAreaOccupied += (coord, size) => ChimeraLogger.Log($"[GridSystem] Area occupied at {coord}");
            _placement.OnAreaFreed += (coord, size) => ChimeraLogger.Log($"[GridSystem] Area freed at {coord}");

            _visualization.OnVisibilityChanged += visible => OnGridVisibilityChanged?.Invoke(visible);

            _heightManager.OnVisibleHeightLevelChanged += level => ChimeraLogger.Log($"[GridSystem] Visible height level changed to {level}");
            _heightManager.OnFoundationRequirementChanged += (coord, required) =>
                ChimeraLogger.Log($"[GridSystem] Foundation requirement changed at {coord}: {required}");
        }

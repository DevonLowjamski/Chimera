...
namespace ProjectChimera.Systems.Addressables
{
    ...
    /// <summary>
    /// REFACTORED: Thin MonoBehaviour wrapper for AddressablesAssetService
    ...
    /// </summary>
    public class AddressablesAssetManagerBridge : MonoBehaviour, IAssetManager
    {
        ...
        private static AddressablesAssetManagerBridge _instance;

        public static AddressablesAssetManagerBridge Instance => _instance;
        ...
        public async Task<T> LoadAssetAsync<T>(string address) where T : UnityEngine.Object
            => await (_service?.LoadAssetAsync<T>(address, Time.realtimeSinceStartup) ?? Task.FromResult<T>(null));

        public T LoadAsset<T>(string assetPath) where T : UnityEngine.Object
            => _service?.LoadAssetSync<T>(assetPath);
        ...
        public void LoadAssetAsync<T>(string assetPath, Action<T> onComplete, Action<string> onError = null) where T : UnityEngine.Object
        {
            ...
            {
                if (t.IsFaulted)
                    onError?.Invoke(t.Exception?.GetBaseException().Message ?? "Load failed");
                else
                {
                    var result = t.Result;
                    if (result != null)
                        ...
                        onError?.Invoke($"Asset '{assetPath}' returned null");
                }
            });
        }

        public Task<IList<T>> LoadAssetsAsync<T>(IList<string> assetPaths) where T : UnityEngine.Object
            => _service?.LoadAssetsAsync<T>(assetPaths, Time.realtimeSinceStartup) ?? Task.FromResult<IList<T>>(new List<T>());

        public Task<IList<T>> LoadAssetsAsync<T>(IList<string> assetPaths, CancellationToken cancellationToken) where T : UnityEngine.Object
            => LoadAssetsAsync<T>(assetPaths);
        ...
        public Task<IList<T>> LoadAssetsByLabelAsync<T>(string label, CancellationToken cancellationToken) where T : UnityEngine.Object
            => LoadAssetsByLabelAsync<T>(label);

        public Task<bool> HasAssetAsync(string assetPath)
            => _service?.HasAssetAsync(assetPath) ?? Task.FromResult(false);

        public Task<bool> HasAssetAsync(string assetPath, CancellationToken cancellationToken)
            => HasAssetAsync(assetPath);
        ...
        public Task PreloadAssetsAsync(string[] assetPaths)
            => _service?.PreloadAssetsAsync(assetPaths, Time.realtimeSinceStartup) ?? Task.CompletedTask;

        public Task PreloadAssetsAsync(string[] assetPaths, CancellationToken cancellationToken)
            => PreloadAssetsAsync(assetPaths);
        ...
        public void UnloadAsset<T>(T asset) where T : UnityEngine.Object
            => _service?.UnloadAsset(asset);

        public bool IsAssetLoaded(string assetPath)
            => _service?.IsAssetLoaded(assetPath) ?? false;

        public AssetManagerStats GetStats()
            => _service?.GetStats() ?? new AssetManagerStats();

        public string GetPerformanceReport()
            => _service?.GetPerformanceReport() ?? "Statistics not available";

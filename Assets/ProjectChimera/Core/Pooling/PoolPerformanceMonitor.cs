...
namespace ProjectChimera.Core.Pooling
{
    ...
    /// </summary>
    public class PoolPerformanceMonitor : MonoBehaviour, ITickable
    {
        ...
        public void Initialize()
        {
            ...
            if (_enableLogging)
            {
                ChimeraLogger.LogInfo("PoolPerformanceMonitor", "Operation completed");
            }
        }
    ...
    private void Awake()
    {
        ...
        private void AnalyzePerformance()
        {
            foreach (var poolName in _poolHistory.Keys)
            {
                ...
                if (_enableLogging && recommendations.Count > 0)
                {
                    ...
                    {
                        ChimeraLogger.LogWarning("PoolPerformanceMonitor", "Operation completed");
                    }
                }
            }
        }
        ...
        private int GetOptimalInitialSize(List<PoolPerformanceSnapshot> history)
        {
            ...
            float avgPeakUsage = (float)history.Average(s => (double)s.CountActive);

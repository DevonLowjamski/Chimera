    ...
    public class ServiceContainerValidator
    {
        ...
        public ServiceHealthReport PerformHealthCheck()
        {
            ...
            foreach (var registration in registrations)
            {
                ...
                {
                    var service = _container.TryResolve(registration.ServiceType);
                    var health = service != null ? ServiceHealthStatus.Healthy : ServiceHealthStatus.Unhealthy;
                    report.ServiceHealth[registration.ServiceType] = health;
                }
                catch (Exception ex)
                {
                    report.ServiceHealth[registration.ServiceType] = ServiceHealthStatus.Critical;
                    report.HealthIssues.Add($"Service {registration.ServiceType.Name}: {ex.Message}");
                }
            }

            report.TotalServices = registrations.Count();
            report.HealthyServices = report.ServiceHealth.Values.Count(h => h == ServiceHealthStatus.Healthy);
            report.UnhealthyServices = report.ServiceHealth.Values.Count(h => h == ServiceHealthStatus.Unhealthy);
            report.CriticalServices = report.ServiceHealth.Values.Count(h => h == ServiceHealthStatus.Critical);
